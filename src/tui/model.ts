...
export function createRootModel(): tea.IModel {
    ...
    const models = [
        createStaticTextModel(["", "steph", ""]),
        createExampleInputModel(),
        createExamplePropsGetterOnlyModel(() => exampleSharedCount),
        createExamplePropsSetterModel(
            () => exampleSharedCount,
            (count) => (exampleSharedCount = count),
        ),
        createListModel("this is a list with options", [
            "option 1",
            "option 2",
            "option 3",
        ]),
...
export function createDynamicTextModel(
    linesGetter: () => string[],
): tea.IModel {
    return {
        ...createEmptyModel(),
...
export function createListModel(title: string, options: string[]): tea.IModel {
    ...
    function onKeypress(keypress: tea.ReadlineKeypress) {
        ...
        if (isUp) selectedIndex = Math.max(0, selectedIndex - 1);
...
export function createExampleInputModel(): tea.IModel {
    ...
    function onKeypress(keypress: tea.ReadlineKeypress) {
        ...
        if (keypress.key.name === "backspace") {
            text =
                text.slice(0, Math.max(0, cursorIndex - 1)) +
    ...
    function render(): tea.RenderResult {
        const lines = [inputPrefix + text, lastInput];

        if (!ownsCursor) return { lines, cursor: null };
...
export function createExamplePropsGetterOnlyModel(
    countGetter: () => number,
): tea.IModel {
    return createDynamicTextModel(() => [
        "count: " + countGetter() + " (this is using shared state)",
    ]);
}

export function createExamplePropsSetterModel(
    countGetter: () => number,
    ...
    return {
        ...
        render: () => ({
            lines: [
                "count: " + countGetter() + " (press any key to increment)",
            ],
            cursor: null,
        }),
    };
